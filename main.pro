...
helper_city(Name, TargetList, Cities, Targets) :-
	...
	find_possible_cities(Head, TargetCityList),
	detect_city(Name, TargetCityList, ActivityInCity),
	union(NameCityList, ActivityInCity, CityHead),
...
detect_city(Name, CityList, Result) :-
	...
	city(Head, _, Activities),
	likes(Name, LikedActivities, _),
	intersection(Activities, LikedActivities, Intersection),
	length(Intersection, Length),
	(member(Head, DislikedCities) -> Result = ResultTail ; (Length > 0 -> Result = [Head|ResultTail] ; Result = ResultTail)).
...
city_helper(Name, MatchedTargets, Cities, Targets) :-
	...
	find_possible_cities(Head, Cities2),
	intersection(Cities1, Cities2, Mutual),
	detect_city(Name, Cities2, CityList1),
	detect_city(Head, Cities1, CityList2),
	union(CityList1, CityList2, Union),
	union(Mutual, Union, CityHead),
	length(CityHead, Length),
	copier(Head, Length, TargetHead),
	append(CityHead, CityTail, Cities),
	append(TargetHead, TargetTail, Targets).
